...
public class StudentExceptionsHelper
{
  ...
  public static ServiceErrorException BadRequest(Guid studentId) => new(HttpStatusCode.BadRequest, $"Student with identifier \"{studentId}\" can only have {StudentCommonValues.SUBJECTS_MAX_NUMBER} subjects linked", "Subjects with different teachers can only be added");
}
